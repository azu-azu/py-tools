"""ETLツール「Find Replace」(Find Any Part of Field) を pandas で再現する検証スクリプト。
...
replace_multiple_found で決まる（ETLツール の ReplaceMultipleFound 設定に対応）。
...
def simulate_find_any_append(
    ...
    *,
    ...
    case_sensitive: bool = True,  # ETLツール の NoCase=False（大小を区別）に対応
    replace_multiple_found: bool = True,  # ETLツール の ReplaceMultipleFound。True=last match、False=first match
    verbose: bool = True,
) -> pd.DataFrame:
...
def main() -> None:
    ...
    result = simulate_find_any_append(
        ...
        replace_multiple_found=True,   # ETLツール の ReplaceMultipleFound
        case_sensitive=True,           # ETLツール の NoCase=False
    )
