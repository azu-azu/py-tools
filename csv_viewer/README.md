...
行頭でも値の右側でも `#` でコメントを書ける。

```ini
# 行頭コメント
display_rows = 30   # 値の右側コメント
```

右側に書く場合は **`#` の前に空白が必要**。この仕様のおかげで、値に含まれる `#` は保持される。

```ini
[filter]
code = A#1          # → 値は "A#1"（空白が無いのでコメント扱いされない）
note = see #12      # → 値は "see" に切られる。値に " #" を含めたい場合は注意
```
